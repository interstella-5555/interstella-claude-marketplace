...
- `~/.ssh/allowed_signers` + global `gpg.ssh.allowedSignersFile` for local signature verification
...
7. Append the account's entry to `~/.ssh/allowed_signers` — `<email> namespaces="git" <pubkey>` (email must match `user.email`); ensure `git config --global gpg.ssh.allowedSignersFile` points at it
8. `gh auth login` for the new account
9. Create `.envrc` in account directory
10. Add key to GitHub (Authentication + Signing)
11. Verify with test commit
...
4. Update the account's line in `~/.ssh/allowed_signers` with the new pubkey (else local `--show-signature` fails)
...
5. Remove the account's line from `~/.ssh/allowed_signers`
6. Remove `.envrc` from the account's directory
7. `gh auth logout <account>`
8. Tell user to remove key from GitHub manually
...
# Must be set (e.g. ~/.ssh/allowed_signers) — without it, local verification errors
ls ~/.ssh/allowed_signers && cat ~/.ssh/allowed_signers
# Must exist with one line per account: <email> namespaces="git" <pubkey>
# Each email must match that account's user.email
```
...
# If it errors "gpg.ssh.allowedSignersFile needs to be configured", the signature
# was still made — fix the allowed_signers setup in Check 4, this is verification-only.
```
Clean up after:
```bash
...
2. **Single or multi-account?**
   ...
   > ```
   > ~/
   > ├── personal/          ← personal-account (primary — also the default everywhere)
   > │   ├── project-a/
   > │   └── project-b/
   > ├── work/              ← work-account (only in this directory)
   > │   ├── project-c/
   > │   └── project-d/
   > └── random-project/    ← personal-account (primary kicks in)
   > ```
   >
   > Git config:
   > ```
   > ~/.gitconfig                ← [user] for personal-account (global default)
   >                             + [includeIf "gitdir:~/work/"]
   > ~/.gitconfig-work           ← [user] for work-account (overrides in ~/work/)
   > ```
   >
   ...
   > ```
   > ~/
   > ├── personal/          ← personal-account (only here)
   > │   ├── project-a/
   > │   └── project-b/
   > ├── work/              ← work-account (only here)
   > │   ├── project-c/
   > │   └── project-d/
   ...
   >                             + [includeIf "gitdir:~/work/"]
   > ~/.gitconfig-personal       ← [user] for personal-account
   > ~/.gitconfig-work           ← [user] for work-account
   > ```
   >
   > Which layout?
...
4. **Key name suffix:**
   ...
   >
   > Examples:
   > - `github_work` → key: `~/.ssh/id_ed25519_github_work`, config: `~/.gitconfig-github_work`
...
- `~/.gitconfig` — directory-based account switching (`includeIf`) + `gpg.ssh.allowedSignersFile`
- `~/.gitconfig-<suffix>` per account — SSH key, signing, identity
- `~/.ssh/allowed_signers` — trusted keys for local signature verification (one line per account)
